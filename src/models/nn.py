...
# ─────────────────────────────────────────────────────────────────────────────
# Multi-Head Attention network for tabular data
# ─────────────────────────────────────────────────────────────────────────────

class _TabAttentionNet(nn.Module):
    """
    Tabular multi-head self-attention network.

    Architecture
    ────────────
    1. Feature embedding  : each of the input_dim scalar features is projected
                            to a vector of size embed_dim, producing a sequence
                            of input_dim tokens → (batch, input_dim, embed_dim).
    2. Transformer encoder: num_attn_layers of multi-head self-attention +
                            feed-forward sub-layers (batch_first=True).
    3. Mean pooling       : average the attended tokens → (batch, embed_dim).
    4. MLP head           : one or more Linear→ReLU layers ending in Linear→1.
    ...
    num_attn_layers: Number of stacked TransformerEncoder layers.
    mlp_hidden     : Hidden layer sizes for the MLP head.
    dropout_rate   : Dropout applied inside the transformer and MLP head.
    """
    ...
    def __init__(
        ...
        # W: (input_dim, embed_dim),  b: (input_dim, embed_dim)
        self.feature_weights = nn.Parameter(torch.randn(input_dim, embed_dim) * 0.02)
    ...
    def forward(self, x: "torch.Tensor") -> "torch.Tensor":
        ...
        # Embed each feature: (batch, input_dim, embed_dim)
        x_embed = (
            x.unsqueeze(-1) * self.feature_weights.unsqueeze(0)
            + self.feature_bias.unsqueeze(0)
        )

        # Self-attention across features: (batch, input_dim, embed_dim)
        attended = self.transformer(x_embed)

        # Mean-pool over the feature (sequence) dimension: (batch, embed_dim)
...
@dataclass
class AttentionNNModel:
    """
    Multi-head self-attention model for tabular regression.

    Treats each input feature as a token, applies Transformer encoder
    layers across features, then mean-pools and passes through an MLP head.
    ...
    num_attn_layers: Number of stacked TransformerEncoder layers.
    mlp_hidden     : Hidden layer sizes for the MLP head after pooling.
    dropout_rate   : Dropout rate inside the transformer and MLP head.
    lr             : Adam learning rate.
    batch_size     : Mini-batch size.
    max_epochs     : Maximum training epochs.
    patience       : Early-stopping patience.
    device         : 'cpu', 'cuda', 'mps', or None for auto-detection.
    """

    name: str = "AttentionNN"
    embed_dim: int = 64
    num_heads: int = 4
    num_attn_layers: int = 2
    mlp_hidden: List[int] = field(default_factory=lambda: [256, 128])
    dropout_rate: float = 0.1
    lr: float = 5e-4
    batch_size: int = 256
    max_epochs: int = 150
    ...
    def fit(
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            if has_val:
                ...
                if no_improve >= self.patience:
                    if verbose:
                        logger.info(
                            "[%s] Early stopping at epoch %d  (val_loss=%.5f, best=%.5f)",
                            self.name, epoch, val_loss, best_val_loss,
                        )
                    break

            if verbose and epoch % 10 == 0:
                msg = f"[{self.name}] Epoch {epoch:>4d}/{self.max_epochs}  train_loss={train_loss:.5f}"
                if has_val:
                    msg += f"  val_loss={val_losses[-1]:.5f}"
                msg += f"  ({time.time()-t0:.1f}s)"
                logger.info(msg)
        ...
        if best_state is not None:
            self._net.load_state_dict({k: v.to(dev) for k, v in best_state.items()})
    ...
    def summary(self) -> str:
        total_params = (
            sum(p.numel() for p in self._net.parameters()) if self._net else 0
        )
        mlp_str = " → ".join(str(h) for h in self.mlp_hidden) + " → 1"
        return (
            f"{self.name}  |  embed={self.embed_dim}  heads={self.num_heads}"
            f"  attn_layers={self.num_attn_layers}  mlp={mlp_str}"
            f"  dropout={self.dropout_rate}  |  {total_params:,} params"
        )
